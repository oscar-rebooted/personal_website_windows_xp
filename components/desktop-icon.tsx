...
export function DesktopIcon({ name, icon, onClick, initialPosition }: DesktopIconProps) {
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (!isDragging) {
      setIsSelected(true)
      onClick() // Execute onClick on single click
    }
  }
  ...
  return (
    <div
      ref={iconRef}
      className="absolute flex w-20 flex-col items-center p-2 text-center cursor-pointer hover:bg-[#0a246a]/20"
      style={{
        left: `${position.x}px`,
        top: `${position.y}px`,
        backgroundColor: isSelected ? "rgba(10, 36, 106, 0.4)" : "transparent",
      }}
      onClick={handleClick}
